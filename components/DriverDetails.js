...
import { Surface, List, Avatar, Portal, Modal, IconButton } from 'react-native-paper'
  ...
  return (
    <Surface style={styles.surface} elevation={2}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} showsHorizontalScrollIndicator={false}>
        <List.Section>
          {Object.keys(informationLabels).map((key, index) => {
            return (
              <List.Item key={index} title={informationLabels[key]} description={driverInformation[key]} titleStyle={styles.title} descriptionStyle={styles.description} right={props => <List.Icon {...props} icon="idcard" />}
              />
            )
          })}
        </List.Section>
